...
import { Gauge, Zap, TrendingDown, Award, AlertTriangle, UserCheck } from "lucide-react";
...
export const DriverProfilingTab: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      {/* Top Banner */}
      <div className="glass-panel rounded-2xl p-4 flex flex-wrap items-center justify-between gap-4 border border-purple-500/20 bg-purple-950/10">
        ...
        <div className="flex items-center gap-2 font-mono text-xs">
          ...
          <span className="text-slate-200 font-bold">{vehicle.driver}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <GlassCard glow={isAggressive ? "crimson" : isModerate ? "amber" : "cyan"} className="text-center">
          ...
          <div className="my-4">
            <div className="text-5xl font-extrabold font-mono tracking-tight text-slate-100">
              <AnimatedNumber value={ai} decimals={3} className={isAggressive ? "text-rose-400" : isModerate ? "text-amber-300" : "text-cyan-300"} />
            </div>
            <div className="text-xs text-slate-400 mt-1 font-mono">Scale: 0.0 (Calm) → 1.0 (Extreme)</div>
          </div>
          <div className="pt-3 border-t border-slate-800 text-xs text-slate-300">
            Cohort: <strong className={isAggressive ? "text-rose-400" : isModerate ? "text-amber-400" : "text-emerald-400"}>
        ...
        <GlassCard glow={bsi > 0.6 ? "crimson" : bsi > 0.4 ? "amber" : "purple"} className="text-center">
          ...
          <div className="my-4">
            <div className="text-5xl font-extrabold font-mono tracking-tight text-slate-100">
              <AnimatedNumber value={bsi} decimals={3} className={bsi > 0.6 ? "text-rose-400" : bsi > 0.4 ? "text-amber-300" : "text-purple-300"} />
            </div>
            <div className="text-xs text-slate-400 mt-1 font-mono">Electrochemical Strain (C-rate + Temp)</div>
          </div>
          <div className="pt-3 border-t border-slate-800 text-xs text-slate-300">
            Peak Current Draw: <strong className="text-amber-300">{maxDischarge} A</strong>
          </div>
        </GlassCard>
        ...
        <GlassCard glow="amber" className="text-center flex flex-col justify-between">
          ...
          <div className="my-3">
            ...
            <div className="text-xs text-slate-400 mt-1 font-mono">Excess degradation per 25,000 km</div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-3">
            <div>
              <div className="flex justify-between text-xs font-mono mb-1">
                <span className="text-slate-400">Harsh Acceleration Events / Trip</span>
                <span className="text-cyan-400 font-bold">{harshAccel}</span>
              </div>
              <input
                type="range"
                min="0"
            ...
            <div>
              <div className="flex justify-between text-xs font-mono mb-1">
                <span className="text-slate-400">Harsh Braking Events / Trip</span>
                <span className="text-amber-400 font-bold">{harshBrake}</span>
              </div>
              <input
                type="range"
                min="0"
            ...
            <div>
              <div className="flex justify-between text-xs font-mono mb-1">
                <span className="text-slate-400">Speed Variance (σ)</span>
                <span className="text-purple-400 font-bold">{speedVariance.toFixed(1)} km/h</span>
              </div>
              <input
                type="range"
                min="2"
            ...
            <div>
              <div className="flex justify-between text-xs font-mono mb-1">
                <span className="text-slate-400">Peak Discharge Current (A)</span>
                <span className="text-rose-400 font-bold">{maxDischarge.toFixed(0)} A</span>
              </div>
              <input
                type="range"
                min="15"
        ...
        <GlassCard className="space-y-4 flex flex-col justify-between">
          <div>
            ...
            <div className="mt-3 space-y-2.5">
              {behaviorData?.recommendations.map((rec, i) => (
                <div key={i} className="p-3 rounded-xl bg-slate-900/60 border border-slate-800 text-xs text-slate-300 flex items-start gap-2">
          ...
          <div className="text-[11px] text-slate-400 pt-3 border-t border-slate-800">
            Formulation: <code className="text-xs text-slate-300 font-mono">AI = 0.25·Accel + 0.20·Brake + 0.15·σ + 0.15·v²</code>
          </div>
        </GlassCard>
      </div>
    </div>
