...
import { Flame, ShieldAlert, Thermometer, ShieldCheck, AlertCircle, RefreshCw } from "lucide-react";
...
export const ThermalSafetyTab: React.FC = () => {
  ...
  // Determine which thermal zone is the most critical to apply the signature pulsing moment
  const maxZone =
        ...
        <GlassCard
          ...
          className={`relative ${
            maxZone === "battery" && (isWarning || isCritical)
              ? "ring-2 ring-rose-500 animate-pulse-fast"
              : ""
          }`}
        >
          ...
          <div className="mt-4 flex items-baseline justify-between">
            <AnimatedNumber
              ...
              decimals={1}
              className={`text-4xl ${
                batteryTemp > 45 ? "text-rose-400" : batteryTemp > 38 ? "text-amber-300" : "text-cyan-300"
              }`}
              suffix="°C"
            />
          ...
          <p className="text-[11px] text-slate-400 mt-2">Core Li-ion cell prism temperature</p>
        ...
        <GlassCard
          ...
          className={`relative ${
            maxZone === "controller" && (isWarning || isCritical)
              ? "ring-2 ring-amber-500 animate-pulse-fast"
              : ""
          }`}
        >
          <div className="flex items-center justify-between text-xs font-semibold uppercase tracking-wider text-slate-400">
          ...
          <div className="mt-4 flex items-baseline justify-between">
            <AnimatedNumber
              ...
              decimals={1}
              className={`text-4xl ${
                controllerTemp > 60 ? "text-rose-400" : controllerTemp > 50 ? "text-amber-300" : "text-emerald-300"
              }`}
              suffix="°C"
            />
          ...
          <p className="text-[11px] text-slate-400 mt-2">IGBT & Inverter switching stage</p>
        ...
        >
          ...
          <div className="mt-4 flex items-baseline justify-between">
            <AnimatedNumber
              ...
              decimals={1}
              className={`text-4xl ${
                motorTemp > 75 ? "text-rose-400" : motorTemp > 65 ? "text-amber-300" : "text-purple-300"
              }`}
              suffix="°C"
            />
          ...
          <p className="text-[11px] text-slate-400 mt-2">PMSM Stator winding sensor</p>
        ...
        <GlassCard glow="emerald" className="flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between">
              ...
              <span className="text-[11px] font-mono text-slate-400">PyTorch 1D-CNN+LSTM</span>
            </div>
            <div className="mt-3">
              <div className="text-xs text-slate-400">10-Step Spatial-Temporal Estimate:</div>
              ...
              <div className="text-xs font-medium text-slate-300 mt-1">
                Classification: <strong className="text-emerald-400">{sohDeepData?.capacity_state || "Optimal"}</strong>
              </div>
            </div>
          </div>
          <div className="text-[11px] text-slate-400 pt-3 border-t border-slate-800">
            Confidence: <strong className="text-slate-200">{((sohDeepData?.confidence_score || 0.95) * 100).toFixed(1)}%</strong>
          </div>
        </GlassCard>
      </div>
    </div>
