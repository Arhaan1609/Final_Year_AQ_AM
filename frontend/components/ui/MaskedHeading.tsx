...
export const MaskedHeading: React.FC<MaskedHeadingProps> = ({
  ...
  const place = useCallback(() => {
    ...
    media.style.transform = `translate3d(${clamp(off.x, -maxX, maxX).toFixed(2)}px, ${clamp(
      off.y,
      -maxY,
      maxY
    ).toFixed(2)}px, 0) scale(${s.fillScale})`;
    media.style.filter = `brightness(${s.brightness}) saturate(${s.saturation}) contrast(1.15)${
  ...
  const sync = useCallback(() => {
    ...
    root.style.fontSize = `${clamp(root.clientWidth * s.textScale, 32, 160).toFixed(1)}px`;
    ...
    for (let i = 0; i < wordRefs.current.length; i += 1) {
      ...
      const x = boxRect.left - rootRect.left;
      // Precise baseline positioning
      const y = boxRect.top - rootRect.top + boxRect.height * 0.81;

      glyph.setAttribute("x", `${x.toFixed(2)}`);
      glyph.setAttribute("y", `${y.toFixed(2)}`);
  ...
  useEffect(() => {
    ...
    const frame = (now: number) => {
      ...
      const dx = Math.sin(clock * 0.25) * s.drift;
      const dy = Math.cos(clock * 0.2) * s.drift * 0.6;
  ...
  return (
    <Tag
      ...
      {...rest}
    >
      {/* 1. Underlying Glow & Readability Stroke */}
      <span
        className="absolute inset-0 block select-none pointer-events-none font-black"
        style={{
          color: "transparent",
          WebkitTextStroke: "1px rgba(56, 189, 248, 0.4)",
          textShadow: "0 0 35px rgba(6, 182, 212, 0.5), 0 0 70px rgba(16, 185, 129, 0.3)",
