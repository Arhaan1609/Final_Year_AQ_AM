...
# 1. Load OLD row-split model for direct apples-to-apples comparison
...
# 2. Select 17 vehicles with diverse characteristics
# (DL1LAN0707 + GJ05CV6564 + 15 vehicles spanning low, mid, high baseline and cycle counts)
with open("frontend/public/data/fleet_vehicles.json", "r") as f:
    all_fleet = json.load(f)

fleet_by_id = {v["id"]: v for v in all_fleet}
fleet_by_chassis = {v.get("chassis"): v for v in all_fleet if "chassis" in v}

selected_specs = [
    # 2 Original vehicles
    {"id": "DL1LAN0707", "chassis": "MAT486022RZD10219", "soh_0": 99.2, "cycles": 0, "voltage": 77.1, "temp": 40.0, "current": -16.5, "model_desc": "Tata Ace EV (Brand New)"},
    {"id": "GJ05CV6564", "chassis": "CN-GJ05CV6564",    "soh_0": 96.9, "cycles": 200, "voltage": 80.4, "temp": 29.4, "current": -1.5, "model_desc": "Euler HiLoad EV (Mid Life)"},
    
    # Low Baseline / High Degradation cohort (79% - 85% SOH)
...
print("=" * 110)
print("17-VEHICLE SPOT-CHECK: CALIBRATED BASELINE MODEL VS OLD ROW-SPLIT MODEL")
print("=" * 110)
header = f"{'Vehicle ID':12s} | {'Chassis No':18s} | {'Cycles':6s} | {'SOH_0 (Base)':12s} | {'Pred d_SOH':10s} | {'New SOH':8s} | {'Old SOH':8s} | {'Model Used':18s}"
print(header)
print("-" * 110)
for _, r in df_res.iterrows():
    print(f"{r['vehicle_id']:12s} | {r['chassis_no']:18s} | {r['cycles']:6.0f} | {r['soh_0']:10.2f}%  | {r['pred_delta']:+8.2f}% | {r['reconstructed_soh']:7.2f}% | {r['old_rowsplit_soh']:7.2f}% | {r['model_used']:18s}")

print("\n" + "=" * 110)
print("SPREAD & VARIANCE ANALYSIS")
print("=" * 110)
print(f"Commissioning Baseline SOH_0 : Min = {df_res['soh_0'].min():.2f}%, Max = {df_res['soh_0'].max():.2f}%, Range = {df_res['soh_0'].max() - df_res['soh_0'].min():.2f}%, Std = {df_res['soh_0'].std():.2f}%")
print(f"Predicted Delta (d_SOH)      : Min = {df_res['pred_delta'].min():+.2f}%, Max = {df_res['pred_delta'].max():+.2f}%, Range = {df_res['pred_delta'].max() - df_res['pred_delta'].min():.2f}%, Std = {df_res['pred_delta'].std():.2f}%")
print(f"NEW Reconstructed SOH        : Min = {df_res['reconstructed_soh'].min():.2f}%, Max = {df_res['reconstructed_soh'].max():.2f}%, Range = {df_res['reconstructed_soh'].max() - df_res['reconstructed_soh'].min():.2f}%, Std = {df_res['reconstructed_soh'].std():.2f}%")
print(f"OLD Row-Split Model SOH      : Min = {df_res['old_rowsplit_soh'].min():.2f}%, Max = {df_res['old_rowsplit_soh'].max():.2f}%, Range = {df_res['old_rowsplit_soh'].max() - df_res['old_rowsplit_soh'].min():.2f}%, Std = {df_res['old_rowsplit_soh'].std():.2f}%")
print("=" * 110)
