...
# Keras / TensorFlow for Deep Learning — MUST be imported before sklearn / xgboost on Windows
TF_AVAILABLE = False
try:
    import keras
    ...
    print(f"  [WARNING] Keras could not be imported: {_e}. DL models will be skipped.")
...
EXCLUDE_COLS_PER_TASK = {
    "SOC": [
        ...
        "voltage_deviation",  # = battery_voltage - 72 (constant offset = duplicate, r=0.9947)
    ],
    "SOH": [
    ...
    "RUL": [
        ...
        "charge_frequency",    # = charge_cycle_count/days_in_service => direct proxy of target
    ],
    "Mileage": [
...
def get_ml_models():
    rs = cfg.TRAIN_CONFIG["random_state"]
    return {
        # Regularized ML architectures to prevent overfitting and ensure fast convergence
...
def build_cnn(seq_len: int, n_features: int):
    model = Sequential([
        Input(shape=(seq_len, n_features)),
...
def build_cnn_lstm(seq_len: int, n_features: int):
    model = Sequential([
        Input(shape=(seq_len, n_features)),
        Conv1D(filters=64, kernel_size=3, activation="relu", padding="same"),
        BatchNormalization(),
...
def train_dl_models(X_train, X_test, y_train, y_test, task: str) -> dict:
    ...
    if not TF_AVAILABLE:
        print_warning(f"  DL models skipped for {task}: Keras/TensorFlow not available.")
        return {}
    results = {}
    ...
    for name, build_fn, use_seq in [
        ...
        if use_seq:
            # For sequence models on huge datasets, train on representative window (up to 50k rows)
            X_tr_raw = X_train.values if hasattr(X_train, "values") else X_train
            y_tr_raw = y_train
            X_ts_raw = X_test.values if hasattr(X_test, "values") else X_test
            y_ts_raw = y_test
            if len(X_tr_raw) > 50000:
                X_tr_raw = X_tr_raw[-50000:]
                y_tr_raw = y_tr_raw[-50000:]
            if len(X_ts_raw) > 10000:
                X_ts_raw = X_ts_raw[-10000:]
                y_ts_raw = y_ts_raw[-10000:]
            Xtr_s, ytr_s = make_sequences(X_tr_raw, y_tr_raw, seq_len)
            Xts_s, yts_s = make_sequences(X_ts_raw, y_ts_raw, seq_len)
            if len(Xtr_s) < 20:
        ...
        batch_size = 256 if len(X_tr_in) > 30000 else cfg.DL_CONFIG["batch_size"]
        epochs = 30 if len(X_tr_in) > 30000 else cfg.DL_CONFIG["epochs"]
