...
def run_fast_eval(results):
        ...
        for split in ["ml","dl"]:
            for mname, mres in task_data.get(split,{}).items():
                ...
                rows.append({
                    ...
                    "Train_R2":  round(trm.get("R2",   float("nan")), 4),   # fix: was missing → FitStatus always "Underfitting"
                    "Train_RMSE":round(trm.get("RMSE", float("nan")), 4),
    ...
    if not comp_df.empty:
        ...
        print("="*70)
        print(comp_df[["Task","Model","Train_R2","R2","RMSE","MAE","FitStatus"]].to_string(index=False))
